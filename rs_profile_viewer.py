...
def print_html():
    ...
    for (id, mastery) in sortedM:
        ...
        datatowrite += "<tr><td>{}</td> <td>{}</td> <td>{}</td> <td>{}</td><td>".format(
            SongDetails["song"], SongDetails["artist"], SongDetails["arrangement"], str(mastery) + "%")
    
        if (mastery > 90):
            datatowrite += "<svg width='100%' height='20%'><rect width='" + \
                str(mastery) + "%' height='100%' style='fill:lightgreen;stroke-width:2;stroke:rgb(0,0,0)'></rect></svg>" + "</td></tr>"
        else:
            datatowrite += "<svg width='100%' height='20%'><rect width='" + \
                str(mastery) + "%' height='100%' style='fill:yellow;stroke-width:2;stroke:rgb(0,0,0)'></rect></svg>" + "</td></tr>"
...
def read_psarc():
    ...
    #os.chdir(dlcPath)
    psarcblacklist = ["session.psarc", "static.psarc", "gears.psarc", "etudes.psarc", "crowd.psarc", "audio.psarc", "video.psarc" ]
    for root, dirs, files in os.walk(basePath):
        for file in files:
            fullpath = os.path.join(root, file)
            if file.endswith(".psarc") and file not in psarcblacklist and "Tmp" not in fullpath:
...
def generate_yt_missing_playlist(playlistID):
    ...
    for id in SongCache:
        SongDetails = SongCache[id]
        if not SongDetails["song"] or not SongDetails["artist"] or SongDetails["sku"] in ["RS1"]:
            continue
        ...
        if(len(rows) == 0):
            if SongDetails["dlc"]:
                print ("No song name match for " + SongDetails["song"] + " isDLC: " + str(SongDetails["dlc"]) + " SKU: " + SongDetails["sku"])
            continue
        row = None
        for potentialrow in rows:
            if unidecode(SongDetails["artist"]).lower() in potentialrow[2].lower():
                row = potentialrow
        if row != None:
            print("for song: " + SongDetails["song"] +
                  " matches: " + str(len(rows)) +
                  " marking db entry: " + row[2] + " owned.." + 
                  " index: " + str(row[0]))
            sql_update_entry = ''' UPDATE songs
                  SET owned = 1
                  WHERE id = ?'''
            c.execute(sql_update_entry,
                      ( row[0],))

        else:
            print ("no song/artist matches for song: " + SongDetails["song"] + " " + SongDetails["artist"] + " isDLC: " + str(
                SongDetails["dlc"]) + " SKU: " + SongDetails["sku"])
    conn.commit()
    ...
    for row in c.fetchall():
        ...
        if(len(splittrack) > 1):
            ...
            if (chosenvideo in videos.keys()):
                print ("Track: " + str(count) + " " + parsedtrack +
                        " YTID: " + str(chosenvideo), end='')
                print (" - Already in playlist")
                playlistItemId = videos[chosenvideo]
            else:
                print ("Track: " + str(count) + " " + parsedtrack + " YTID: " + str(chosenvideo), end='')
                response = add_to_playlist(youtube, playlist, chosenvideo)
                playlistItemId = response["id"]
                print (" - Added to playlist")

            if (row[6] == 1): #ignore tracks
                response = remove_from_playlist(youtube, playlist, playlistItemId)
                print ("Track: " + str(count) + " " + parsedtrack +
                       " YTID: " + str(chosenvideo) + " ItemID: " + playlistItemId, end='')
...
def add_to_playlist(youtube, playlist, chosenvideo):
    return playlist_items_insert(youtube,
...
def remove_from_playlist(youtube, playlist, itemid):
    return playlist_items_delete(youtube,
                                 id=itemid,
                                 onBehalfOfContentOwner='')
...
def mark_song(a, v):
    ...
    conn = sqlite3.connect("rocksmithdlc.db")
    c = conn.cursor()
    c.execute("update songs set ignore=? where name LIKE ?" , (v,"%" + a+ "%"))
    conn.commit()
    c = conn.cursor()
...
def print_dlc_stats():
    ...
    c.execute("select count(*) from songs where name NOT LIKE '%SONG PACK%' AND owned=1")
    ...
    print ("Total DLC(steam): {} Excluding SongPacks: {} Owned: {} Not Owned: {}".format(total_dlc, total_dlc_no_sp, total_owned, total_not_owned))
